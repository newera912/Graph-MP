...
public class TransWeatherRealDataTest {
	...
	public static void testSingleFileWinMean(String singleFile, String resultFileName) {
...
//					}
				...
				//double[] avgAbsSlope=getAvgAbsSlope(X, S); //mean[k]/winNormalizeFactor[k]
				for(int k=0;k<X.length;k++) winMean[k]=mean[k];
				...
				resultMap.put(Math.round(1000*graphMP.funcValue)/1000.0D,"["+ ArrayUtils.toString(graphMP.resultNodes_Tail).replace("{", "").replace("}", "")+"] ["+ArrayUtils.toString(S).replace("{", "").replace("}", "")+"] "+Arrays.toString(XX).replace("{", "").replace("}", ""));
...
//					}
				...
				for(int k=0;k<X.length;k++) winMean[k]=1;
				...
				//Utils.stop();
				//System.out.println(graphMP.funcVal+ " "+"["+ ArrayUtils.toString(graphMP.resultNodes_Tail).replace("{", "").replace("}", "")+"] ["+ArrayUtils.toString(S).replace("{", "").replace("}", "")+"]");
				resultMap.put(Math.round(1000*graphMP.funcValue)/1000.0D,"["+ ArrayUtils.toString(graphMP.resultNodes_Tail).replace("{", "").replace("}", "")+"] ["+ArrayUtils.toString(S).replace("{", "").replace("}", "")+"] "+Arrays.toString(XX).replace("{", "").replace("}", "")+" \n"+Arrays.toString(winNormalizeFactor).replace("{", "").replace("}", ""));
