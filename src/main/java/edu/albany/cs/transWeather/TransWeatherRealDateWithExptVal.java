...
public class TransWeatherRealDateWithExptVal {
	...
	public static void testSingleFileChangePoint(String singleFile, String resultFileName,String gtFileName,String prf1File,int mwin,int sss,double[] expAvgs) {
...
//				}
	 ...
	 * 
	\*********************************/
	public static void testSingleFileChangePoint2(String singleFile, String resultFileName,String gtFileName,String prf1File,int mwin,int sss,double[] expAvgs) {
		long startTime = System.nanoTime();
		FileWriter fileWriter=null;
		FileWriter prf1Writer=null;
		APDMInputFormat apdm = new APDMInputFormat(singleFile);
...
//				}
	...
	public static double[] getExpAvg(String fileName){
		double[] expAvg=new double[10];
		int idx=0;
		try {
			for (String eachLine : Files.readAllLines(Paths.get(fileName))) {
				if(eachLine.trim().length()<1){
				...
				expAvg[idx++]=Double.parseDouble(eachLine.trim().split(" ")[0]);
	...
	public static void CaseStudy(int maxwin,int sss){
		...
		for(File apdmFile : new File(folder).listFiles()){
			...
			double[] expAvgs=getExpAvg("data/mesonet_data/mesonetExpData/statExpData/"+date+".txt");
			...
			if(methodType.equals("CP")){
				...
				testSingleFileChangePoint(folder+apdmFile.getName(), outFile,"outputs/mesonetPlots/temp_CaseStudy/true_values3.txt",prf1File,maxwin,sss,expAvgs);
			}else{
				System.out.println("---CP3---");
				testSingleFileChangePoint2(folder+apdmFile.getName(), outFile,"outputs/mesonetPlots/temp_CaseStudy/true_values3.txt",prf1File,maxwin,sss,expAvgs);
		...
		String prf1Filepath=sss+"/prf1/expt_PRF1_result-"+methodType+"_ExpBaseMeanDiff_20_s_"+sss+"_wMax_"+maxwin+"_filter_TIncld_0.7.txt";
		String resultFilepath=sss+"/expt_TopK_result-"+methodType+"_ExpBaseMeanDiff_20_s_"+sss+"_wMax_"+maxwin+"_filter_TIncld_0.7.txt";
		try {
	    ...
	    {
	    	ResultItem.printItem(allResultList.get(i));
	...
	public static void main(String[] args) {
		...
		List<Integer> maxWin=(List) Arrays.asList(12,18,24,30,36);//,24,36);//,12,18,24,30,36);
		List<Integer> ss=(List) Arrays.asList(2,3);
		for(int mwin:maxWin){
			for(int sss:ss){
				CaseStudy(mwin,sss);
			}
